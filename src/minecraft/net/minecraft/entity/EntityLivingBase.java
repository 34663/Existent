...
public abstract class EntityLivingBase extends Entity {
    ...
    private int getArmSwingAnimationEnd() {
        ...
        final int ArmSpeed = animations.isEnabled() ? ((Number) animations.getSetting(Animations.SPEED).getValue()).intValue() : 6;

        if (this.isPotionActive(MobEffects.HASTE)) {
            return ArmSpeed - (1 + this.getActivePotionEffect(MobEffects.HASTE).getAmplifier());
        } else {
            return this.isPotionActive(MobEffects.MINING_FATIGUE) ? ArmSpeed + (1 + this.getActivePotionEffect(MobEffects.MINING_FATIGUE).getAmplifier()) * 2 : ArmSpeed;
        }
    }
    ...
    public void handleStatusUpdate(byte id) {
        ...
        if (id != 2 && !flag && !flag1 && !flag2) {
            ...
            EventDamage eventDamage = new EventDamage(this);
            eventDamage.call();
            this.limbSwingAmount = 1.5F;
            this.hurtResistantTime = this.maxHurtResistantTime;
            this.maxHurtTime = 10;
     ...
     */
    protected void jump() {
        double ymot = this.isPotionActive(Potion.getPotionById(8)) ? getJumpUpwardsMotion() + (getActivePotionEffect(Potion.getPotionById(8)).getAmplifier() + 1) * 0.1F : getJumpUpwardsMotion();
        EventJump ej = new EventJump(ymot, true);
        EventJump ej1 = new EventJump(ej.getMotionY(), false);
        ej.call();
        if (ej.isCancelled()) {
            return;
        }
