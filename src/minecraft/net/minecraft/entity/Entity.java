...
import zyx.existent.event.events.EventEntityUpdate;
...
public abstract class Entity implements ICommandSender {
    ...
    public void moveEntity(MoverType x, double p_70091_2_, double p_70091_4_, double p_70091_6_) {
        EventEntityUpdate entityUpdate = new EventEntityUpdate();
        entityUpdate.call();
        ...
        if (this.noClip) {
            ...
            if (((x == MoverType.SELF || x == MoverType.PLAYER) && this.onGround && this.isSneaking() && this instanceof EntityPlayer && !Scaffold.isSneaking) || (eventSafeWalk.isCancelled() && this instanceof EntityPlayer)) {
    ...
    public float getCollisionBorderSize() {
        final HitBox hitBox = (HitBox) Existent.getModuleManager().getClazz(HitBox.class);
        float size;

        if (hitBox.isEnabled()) {
            size = 0.1F + ((Number) hitBox.getSetting(HitBox.SIZE).getValue()).floatValue();
        } else {
            size = 0.0F;
        }
        return size;
