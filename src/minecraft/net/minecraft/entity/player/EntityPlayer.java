...
import zyx.existent.event.events.EventInsideBlock;
...
public abstract class EntityPlayer extends EntityLivingBase {
    ...
    public float getDigSpeed(IBlockState state) {
        ...
        SpeedMine speedMine = (SpeedMine) Existent.getModuleManager().getClazz(SpeedMine.class);
        ...
        if (this.isPotionActive(MobEffects.HASTE) && !(speedMine.isEnabled() && (Boolean) speedMine.getSetting(SpeedMine.HASTE).getValue())) {
        ...
        if (this.isInsideOfMaterial(Material.WATER) && !EnchantmentHelper.getAquaAffinityModifier(this) && !(speedMine.isEnabled() && (Boolean) speedMine.getSetting(SpeedMine.WATER).getValue())) {
            f /= 5.0F;
        }

        if (!this.onGround && !(speedMine.isEnabled() && (Boolean) speedMine.getSetting(SpeedMine.GROUND).getValue())) {
            f /= 5.0F;
        }
    ...
    public void attackTargetEntityWithCurrentItem(Entity targetEntity) {
        BetterSounds betterSounds = (BetterSounds) Existent.getModuleManager().getClazz(BetterSounds.class);
...
//                            this.setSprinting(false);
                        ...
                        if (flag3) {
                            ...
                            if (!(betterSounds.isEnabled() && (Boolean) betterSounds.getSetting(BetterSounds.FIGHT).getValue()))
    ...
    public boolean isEntityInsideOpaqueBlock() {
        ...
        insideBlock.call();
        return !this.sleeping && super.isEntityInsideOpaqueBlock() && !insideBlock.isCancelled();
    }
