...
public class ItemRenderer {
    ...
    private void renderMapFirstPerson(ItemStack stack) {
        ...
        tessellator.draw();
        MapData mapdata = ReflectorForge.getMapData(Items.FILLED_MAP, stack, this.mc.theWorld);
    ...
    private void renderArmFirstPerson(float p_187456_1_, float p_187456_2_, EnumHandSide p_187456_3_) {
        ...
        Chams chams = (Chams) Existent.getModuleManager().getClazz(Chams.class);
        boolean flag2 = (chams.isEnabled() && (Boolean) chams.getSetting(Chams.HAND).getValue());
        ...
        if (flag2) {
            ...
            GL11.glColor4f(255 / 255.0F, 60 / 255.0F, 60 / 255.0F, 40 / 255.0F);
            GL11.glDisable(2929);
        ...
        if (flag2) {
            ...
            GL11.glColor4f(255 / 255.0F, 70 / 255.0F, 70 / 255.0F, 40 / 255.0F);
            if (flag) {
                renderplayer.renderRightArm(abstractclientplayer);
            } else {
                renderplayer.renderLeftArm(abstractclientplayer);
    ...
    private void transformFirstPerson(EnumHandSide p_187453_1_, float p_187453_2_) {
        Module animations = Existent.getModuleManager().getClazz(Animations.class);
        int i = p_187453_1_ == EnumHandSide.RIGHT ? 1 : -1;

        float l = 0;
        l += Math.floor(spin1(3.0));
        boolean flag = (animations.isEnabled() && (Boolean) animations.getSetting(Animations.ROTATION).getValue());
        if (!flag) {
            ...
            GlStateManager.rotate(l, 1.0f, 1.0f, 0.0f);
        }

        if ((Boolean) animations.getSetting(Animations.MINI).getValue()) {
    ...
    private void func_178105_d(final float p_178105_1_) {
        final float f = -0.4f * MathHelper.sin(MathHelper.sqrt(p_178105_1_) * 3.1415927f);
        final float f2 = 0.2f * MathHelper.sin(MathHelper.sqrt(p_178105_1_) * 3.1415927f * 2.0f);
        final float f3 = -0.2f * MathHelper.sin(p_178105_1_ * 3.1415927f);
        GlStateManager.translate(f, f2, f3);
    }

    private void func_178103_c() {
    ...
    private void transformFirstPersonItem2(final float equipProgress, final float swingProgress) {
        Module animations = Existent.getModuleManager().getClazz(Animations.class);
        ...
        if (!flag) {
            final float f = MathHelper.sin(swingProgress * swingProgress * 3.1415927f);
            final float f2 = MathHelper.sin(MathHelper.sqrt(swingProgress) * 3.1415927f);
            GlStateManager.rotate(f * -8.0f, 0.0f, 0.0f, 0.0f);
            GlStateManager.rotate(f2 * -8.0f, 0.0f, 0.0f, 1.0f);
            GlStateManager.rotate(f2 * -30.0f, 0.01f, 0.0f, 0.0f);
        }
    ...
    private void func_178096_A(float p_178096_1_, float p_178096_2_) {
        ...
        boolean flag = (animations.isEnabled() && (Boolean) animations.getSetting(Animations.ROTATION).getValue());
        GlStateManager.translate(0.62f, -0.44f, -0.71999997f);
    ...
    public void renderItemInFirstPerson(float partialTicks) {
        ...
        if (flag1 && !(itemStackOffHand.getItem() instanceof ItemShield && (animations.isEnabled() && (Boolean) animations.getSetting(Animations.OLDANIM).getValue()))) {
        ...
        if (!Config.isShaders() || !Shaders.isSkipRenderHand(p_187457_4_)) {
            ...
            if (p_187457_6_.func_190926_b()) {
                ...
                if (p_187457_1_.isHandActive() && p_187457_1_.getItemInUseCount() > 0 && p_187457_1_.getActiveHand() == p_187457_4_) {
                    ...
                    float swingprogress = (Boolean) animations.getSetting(Animations.ROTATION).getValue() ? 0 : mc.thePlayer.getSwingProgress(p_187457_2_);
                    float var9 = MathHelper.sin(MathHelper.sqrt(swingprogress) * 3.1415927F);
                    float var15 = MathHelper.sin(MathHelper.sqrt(swingprogress) * (float)Math.PI);
                    float l = 0;
                    l += Math.floor(spin2(25.0));

                    if (animations.isEnabled()) {
                        if ((Boolean) animations.getSetting(Animations.OLDANIM).getValue() && (mc.gameSettings.keyBindUseItem.isKeyDown() || mc.thePlayer.isBlocking()) && p_187457_1_.getActiveItemStack().getItem() instanceof ItemShield && p_187457_1_.getActiveHand() == EnumHand.OFF_HAND) {
                            switch (((Options) animations.getSetting(Animations.MODE).getValue()).getSelected()) {
                                case "1.7":
                                    this.transformFirstPersonItem(equipProgress / 3, swingprogress);
                                    this.func_178103_c();
                                    break;
                                case "Mini":
                                    this.transformFirstPersonItem2(equipProgress / 3, swingprogress / 2);
                                    this.func_178103_c();
                                    break;
                                case "MiniJump":
                                ...
                                case "Pump":
                                    ...
                                    GlStateManager.rotate(30.0f, 0.0f, 0.0f, 15.0f);
                                ...
                                case "Spin":
                                    this.transformFirstPersonItem(equipProgress / 2, 0);
                                    this.func_178103_c();
                                    if (mc.thePlayer.getSwingProgress(p_187457_2_) > 0.1) {
                                        GlStateManager.rotate(20, 1, 0, 0);
                                        GlStateManager.rotate(l, 0, 2, 0);
                                    }
                                    break;
                                case "Slide":
                                    ...
                                    break;
                                case "Ejaculation":
                                    transformFirstPersonItem(equipProgress / 2, swingprogress);
                                    func_178103_c();
                                ...
                                case "Matt":
                                    ...
                                    break;
                                case "Swaing":
                                    this.func_178096_A(f / 2, -0.2f);
                                    var15 = MathHelper.sin(swingprogress * swingprogress * 3.1415927f);
                                ...
                                case "Hanabi":
                                    this.transformFirstPersonItem(equipProgress / 2, 0);
                                    final float var16 = MathHelper.sin(MathHelper.sqrt(swingprogress) * 3.1415927f);
                            ...
                            if (!((Boolean) animations.getSetting(Animations.SMOOTH).getValue() || (Boolean) animations.getSetting(Animations.ROTATION).getValue())) {
