...
//        this.renderPotionEffects(scaledresolution);
        ...
        if (this.titlesTimer > 0) {
            ...
            NoTitle noTitle = (NoTitle) Existent.getModuleManager().getClazz(NoTitle.class);
            ...
            if (i2 > 8) {
                ...
                if (!(noTitle.isEnabled() && (Boolean) noTitle.getSetting(NoTitle.MAIN).getValue()))
                ...
                if (!(noTitle.isEnabled() && (Boolean) noTitle.getSetting(NoTitle.SUB).getValue()))
        ...
        zyx.existent.module.modules.visual.ScoreBoard scoreBoard = (zyx.existent.module.modules.visual.ScoreBoard) Existent.getModuleManager().getClazz(zyx.existent.module.modules.visual.ScoreBoard.class);

        if (scoreobjective1 != null && !(scoreBoard.isEnabled() && ((Boolean) scoreBoard.getSetting(ScoreBoard.REMOVE).getValue()))) {
    ...
    protected void renderHotbar(ScaledResolution sr, float partialTicks) {
        if (this.mc.getRenderViewEntity() instanceof EntityPlayer) {
            if (HUD.hotbar) {
                ...
                this.animationX = AnimationUtil.getAnimationState(this.animationX, x, Math.max(50.0F, Math.abs(this.animationX - x) * 10.0F));
                Gui.drawRect(i - 90, sr.getScaledHeight() - 23, i + 92, sr.getScaledHeight(), new Color(0, 0, 0, 150).getRGB());
                Gui.drawRect(this.animationX, sr.getScaledHeight() - 23, this.animationX + 22.0F, sr.getScaledHeight(), new Color(255, 255, 255, 150).getRGB());
    ...
    private void renderScoreboard(ScoreObjective objective, ScaledResolution scaledRes) {
        ...
        for (Score score1 : collection) {
            ...
            int k = scoreBoard.isEnabled() ? j1 - j * this.getFontRenderer().FONT_HEIGHT + ((Number) scoreBoard.getSetting(ScoreBoard.POSITION).getValue()).intValue() : j1 - j * this.getFontRenderer().FONT_HEIGHT;
            int l = scaledRes.getScaledWidth() - 5 + 2;
            drawRect(l1 - 2, k, l, k + (getFontRenderer()).FONT_HEIGHT, 1342177280);
            this.getFontRenderer().drawString(s1, l1, k, 553648127);
            if (!scoreBoard.isEnabled() || !((Boolean) scoreBoard.getSetting(ScoreBoard.POINTS).getValue()))
