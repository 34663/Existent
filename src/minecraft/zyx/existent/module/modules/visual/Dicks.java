...
public class Dicks extends Module {
    ...
    public Dicks(String name, String desc, int keybind, Category category) {
        super(name, desc, keybind, category);

        settings.put(DANCE, new Setting(DANCE, false, "?"));
    }
    ...
    public void esp(EntityPlayer player, double x, double y, double z) {
        ...
        GL11.glRotatef((float) (player.isSneaking() ? 35 : 0) + this.spin, 1.0F + this.spin, 0.0F + ((Boolean) settings.get(DANCE).getValue() ? this.cumSize : 0.0F), 0.0F + ((Boolean) settings.get(DANCE).getValue() ? this.cumSize : 0.0F));
        int lines = 20;
